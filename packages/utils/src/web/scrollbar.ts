...
function measureScrollbarSize(ele?: HTMLElement) {
  ...
  if (ele) {
    const targetStyle = window.getComputedStyle(ele) as ExtendCSSStyleDeclaration
...
export function getScrollbarSize(target: HTMLElement) {
  if (typeof document === 'undefined' || !target || !(target instanceof Element)) return { width: 0, height: 0 }
