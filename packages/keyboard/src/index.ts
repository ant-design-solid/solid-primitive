...
export interface MakeKeyboardOptions {
...
interface MakeKeyboardReturn {
  bind: (key: KeyCombo, action: VoidFunction, options?: KeyboardBindingOptions) => VoidFunction
  unbind: (key: KeyCombo) => void
  trigger: (key: KeyCombo) => void
  reset: VoidFunction
}

export function makeKeyboard(options: MakeKeyboardOptions = {}): MakeKeyboardReturn {
  if (isServer) {
    return {
      bind: noop,
      unbind: noop,
      trigger: noop,
      reset: noop,
    } as unknown as MakeKeyboardReturn
  }
  const { ignore = defaultIgnoreCallback, sequenceTimeout = DEFAULT_SEQUENCE_TIMEOUT, window = defaultWindow } = options
  ...
  const off = makeEventListener(window!, 'keydown', handleKeyDown as EventListener)
...
export function createShortcut(
  ...
  createEffect(() => {
    ...
    if (!isActive() || combos.length === 0) {
      off = noop
      return
    }

    off = makeEventListener(window, 'keydown', (e: KeyboardEvent) => {
      const eventCombo = getEventCombo(e)
      const comboText = comboToId(eventCombo) || null
      const target = getEventTargetElement(e)
